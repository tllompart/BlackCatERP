...
# Thirdparty Library
from app.base.models import WebsiteConfig
...
def get_app_list(obj):
    apps_1 = Plugin.objects.all().filter(installed=True).order_by('sequence')
    return [app.name.lower() + "/menu.html" for app in apps_1]
...
def web_show_register_user(obj):
    try:
        return WebsiteConfig.objects.get(pk=1).user_register
    except WebsiteConfig.DoesNotExist:
        return None
