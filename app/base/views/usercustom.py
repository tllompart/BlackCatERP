...
from ..forms import (
    ...
    PersonaCreationForm)
from ..models import (
    Company, ERPmeta, parameter, Plugin, ERPUser)
...
def _count_plugin():
    return Plugin.objects.all().count()
...
def _web_meta():
    cad = ''
    for meta in ERPmeta.objects.all():
...
class ActivateUserView(RedirectView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            user = ERPUser.objects.get(pk=uid)
        except (TypeError, ValueError, OverflowError, ERPUser.DoesNotExist):
            user = None
...
class AvatarUpdateView(LoginRequiredMixin, FatherUpdateView):
    ...
    success_url = reverse_lazy('ERPUser:profile')
    ...
    def post(self, request, *args, **kwargs):
        ...
        """
        usuario = ERPUser.objects.get(pk=self.request.user.pk)
        ...
        return redirect('ERPUser:profile')
...
def cambio_clave(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and ERPUser.pk != 1:
            user = form.save()
...
class PasswordRecoveryView(PasswordResetView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'uidb64' in self.kwargs and 'token' in self.kwargs:
            ...
            context['url_action'] = reverse_lazy('ERPUser:password-recovery')
        ...
        if 'uidb64' in self.kwargs and 'token' in self.kwargs:
            ...
            if user is not None:
                ...
                return HttpResponseRedirect(
                    reverse_lazy('ERPUser:password-recovery')
                )
