...
class ERPUser(AbstractUser, ERPBase):
    ...
    #avatar = models.ImageField(max_length=255, upload_to=image_path, blank=True, null=True, default='avatar/default_avatar.png')
    partner_id = models.ForeignKey(Partner, null=True, blank=True, on_delete=models.PROTECT)
    active_company = models.ForeignKey('Company', on_delete=models.PROTECT)
    ...
    def create(cls, first_name, last_name, email, password, is_superuser, is_staff, is_active, active_company):
        ...
        partner = Partner.create(partner_name, email)
        ERPuser = cls(
        ...
        ERPUser.set_password(ERPUser, password)
        ERPuser.save()

        return ERPuser

    def create_superuser(self, email, full_name, profile_picture, password=None, username='tllompart', **extra_fields):
        if not email:
