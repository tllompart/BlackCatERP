...
from ...base.models import Plugin
...
def PluginUpdate(self):
    ...
    plugin_list = tuple(
        set(name['name'] for name in Plugin.objects.all().values('name'))
    )
    ...
    return redirect(reverse('Plugin:list'))
...
def PluginInstall(self, pk):
    ...
    try:
        # Se ejecutan las migraciones de la app
        call_command('loaddata', '{}.json'.format(
            plugin.name.lower()), interactive=False)
    except:
        # plugin.installed = False
